...
export default function OutputPage() {
  ...
  const { groupedSupports, validationResult, currentProjectId, currentProjectName, setValidationResult, setGroupedSupports } = useSupportContext()
  const { user } = useAuth()
  ...
  const approvalSubmitted = useRef(false)

  const projectId = currentProjectId
  const projectName = currentProjectName

  // Submit for admin approval when page loads
  useEffect(() => {
    if (approvalSubmitted.current || !groupedSupports || !validationResult) return
    approvalSubmitted.current = true
    ...
    setSubmitted(true)
  }, [groupedSupports, validationResult, submitForApproval, user, projectId, projectName])
  ...
  const handleDownloadPDF = async (type: string) => {
    ...
    try {
      ...
      const blob = await generatePDF(type, rows, projectName)
    ...
    try {
      const pdfs = await Promise.all(
        types.map(async ([type, rows]) => ({
          name: `${type}-supports`,
          blob: await generatePDF(type, rows, projectName),
        }))
      )
