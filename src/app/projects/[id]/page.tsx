...
interface PdfVersion {
  ...
  rowCount: number
  typeCount: number
  /** Tag numbers from the rows snapshot. Used at view time to match
   *  against project uploads so the history entry can name the Excel
   *  file(s) that contributed to this PDF and who uploaded them. */
  supportKeys?: string[]
}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
            ...
            {pdfVersions.length === 0 ? (
              ...
              pdfVersions.map((v, idx) => {
                // Match this version's tag numbers against the project's
                // uploads to figure out which Excel file(s) contributed
                // to the snapshot. An upload counts as "contributing"
                // when at least one of its supportKeys appears in the
                // PDF version. Old versions without supportKeys (pre-
                // migration generations) fall through with no source.
                const versionKeys = new Set(v.supportKeys ?? [])
                const sources = versionKeys.size > 0
                  ? uploads.filter((u) => (u.supportKeys || []).some((k) => versionKeys.has(k)))
                  : []
                return (
                  <div key={v.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 220 }}>
                      ...
                      <div style={{ fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "var(--color-text-muted)", marginTop: 2 }}>
                        Generated by <strong style={{ color: "var(--color-text)" }}>{v.generatedBy}</strong> · {v.rowCount} supports · {v.typeCount} type{v.typeCount !== 1 ? "s" : ""}
                      </div>
                      {sources.length > 0 && (
                        <div style={{ fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "var(--color-text-muted)", marginTop: 2 }}>
                          From {sources.map((u, i) => (
                            <span key={u.id}>
                              {i > 0 && ", "}
                              <strong style={{ color: "var(--color-text)" }}>{u.fileName}</strong>
                              {u.uploadedBy && u.uploadedBy !== "unknown" && (
                                <> (uploaded by <strong style={{ color: "var(--color-text)" }}>{u.uploadedBy}</strong>)</>
                              )}
                            </span>
                          ))}
                        </div>
